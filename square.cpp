    ...
    m_color(1,0,0,1), m_vao(NULL), m_vbo(NULL), m_width(width)
{
    ...
    float d = 1.0;
    vertices[0]=vec3(-d, d,-1); texCoords[0]=vec2(0,0);
    vertices[1]=vec3(-d,-d,-1); texCoords[1]=vec2(0,1);
    vertices[2]=vec3( d, d,-1); texCoords[2]=vec2(1,0);
    ...
    if(initVBO()){
        ...
        m_vbo->write(4*sizeof(vec3)+4*sizeof(vec2),normals,4*sizeof(vec3));
        m_vbo->release();
    }
...
void Square::draw(QOpenGLShaderProgram* prog){
    ...
    prog->setAttributeBuffer("vNormal",GL_FLOAT,4*sizeof(vec3)+4*sizeof(vec2),3,0);
